...
struct Circulo {
	...
	bool operator==(const Circulo& cmp) const {
		if( c == cmp.c && Igual( r, cmp.r ) ) return true;
		return false;
	}
...
int main(){
	...
	while( cin >> x >> y >> r ){
		a = Circulo( Punto( x, y ), r );
		cin >> x >> y >> r;
		b = Circulo( Punto( x, y ), r );

		if( a == b && !(Igual( a.r, 0.0 ))){
			cout << "THE CIRCLES ARE THE SAME\n";
			continue;
		}

		vector< Punto > inter = PuntosInterseccionCirculos( a, b );
		if( inter.size() == 0){
			...
			for( int i = 0; i < inter.size(); i++ ){
				cout << "(" << inter[i].x+eps2 << "," << inter[i].y+eps2 << ")";
			}
			cout << '\n';
		}
	}
